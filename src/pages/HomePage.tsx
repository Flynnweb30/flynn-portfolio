...
import { CASE_STUDIES, CORE_SERVICES, TESTIMONIALS } from '../data/portfolioData';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {CASE_STUDIES.slice(0, 2).map((cs, i) => (
            <motion.button key={cs.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: '-60px' }} transition={{ duration: 0.5, delay: i * 0.1 }} onClick={() => onSelectCaseStudy(cs)} className="group text-left bg-slate-900/60 hover:bg-slate-900/90 backdrop-blur-sm border border-slate-700/60 hover:border-slate-600/80 rounded-xl p-7 sm:p-8 transition-all">
              ...
              <h3 className="text-[19px] sm:text-[21px] font-semibold text-white leading-snug mb-5 group-hover:text-orange-50 transition-colors">{cs.title}</h3>
              <div className="flex items-baseline gap-3 pb-6 mb-6 border-b border-slate-700/60"><span className="text-[26px] sm:text-[32px] font-bold text-orange-400 tabular tracking-tight leading-none">{cs.headlineMetric}</span><span className="text-[11.5px] text-slate-500 font-mono">headline outcome</span></div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mt-14">
          {TESTIMONIALS.slice(0, 3).map((t, i) => (
            <motion.div key={t.id} initial={{ opacity: 0, y: 16 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.5, delay: i * 0.08 }} className="bg-slate-900/60 backdrop-blur-sm border border-slate-700/60 rounded-xl p-7">
              <div className="flex items-center gap-1 mb-5">{[...Array(5)].map((_, j) => <Star key={j} className="w-3 h-3 fill-orange-400 text-orange-400" />)}</div>
      ...
      {/* ── CONTACT ── */}
      <ContactSection onSuccessToast={onSuccessToast} />
